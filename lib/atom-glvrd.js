...
import Glavred from "glvrd-http-api";
...
export default {
    ...
    checkText(editor) {
        ...
        return new Promise((resolve, reject) => {
            ...
            for (var i = 0; i < regions.length; i++) {
                var promise = that.doGlvrdRequest(editor, regions[i]);
    ...
    getGlavredApi() {
        ...
        if (!this.glvrdApi[cacheKey]) {
            this.glvrdApi[cacheKey] = new Glavred("Atom plugin");
        }
        return this.glvrdApi[cacheKey];
    },

    doGlvrdRequest(editor, region) {
        var that = this;
        return new Promise((resolve, reject) => {
            var glavred = that.getGlavredApi();
            glavred.getStatus().then((result) => {
                glavred.proofread(region.text).then((result) => {
