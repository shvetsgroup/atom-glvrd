...
export default {
    ...
    check() {
        ...
        this.checkText(editor).then(function () {
            ...
            let cursorDisposable = editor.onDidChangeCursorPosition((event) => {
                ...
                var markers = layers.errors.findMarkers({valid: true, containsPoint: event.newBufferPosition});
                if (markers.length) {
                    this.glvrdView.setHint(markers[0].getProperties().fragment.hint);
                }
                else {
    ...
    checkText(editor) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (!regions.length) {
                that.updatePanel(editor, that.updateScore(editor));
                return resolve();
            }

            this.checkKeys().then((credentials) => {
                var promises = [];
    ...
    updateScore(editor) {
        ...
        return this.getGlavredApi().getScore(regions);
    },
